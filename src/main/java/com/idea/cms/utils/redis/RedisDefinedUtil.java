package com.idea.cms.utils.redis;

import com.idea.cms.utils.SerializeUtil;
	...
	*/
    ...
    public <T> void setexObj(String keyPrefix,String key,int expireSecond,T obj){
		...
		try{
			...
			byte[] bytes = SerializeUtil.serialize(obj);//序列化
	...
	public <T> T getObj(String keyPrefix,String key,Class<T> tClass){
		...
		try{
			...
			if(bytes!=null){
				return (T)SerializeUtil.unserialize(bytes);//序列化
			}
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		} finally {
